...
class ItemsController extends Controller
{
    ...
    {
        $item->load(['category', 'pricing', 'inventory', 'taxDetail', 'compliance']);

        return Inertia::render('inventory/ItemsShow', [
            'item' => $item,
        ]);
    }
    ...
    public function edit(Item $item): \Inertia\Response
    {
        ...
        return Inertia::render('inventory/ItemsEdit', [
            'item' => $item,
            'categories' => Category::query()->orderBy('name')->get(['id', 'name']),
        ]);
    }

    public function update(UpdateInventoryItemRequest $request, Item $item): \Illuminate\Http\RedirectResponse|\Inertia\Response
    {
        try {
            $validated = $request->validated();
            ...
            return back()->withErrors([
                'general' => 'An error occurred while updating the item: '.$e->getMessage(),
            ]);
        }
    }
    ...
    public function stockIn(StockMovementRequest $request, Item $item): \Illuminate\Http\RedirectResponse
    {
        ...
        if (! $inventory) {
            return back()->withErrors(['general' => 'Item has no inventory record.']);
        }
    ...
    public function stockOut(StockMovementRequest $request, Item $item): \Illuminate\Http\RedirectResponse
    {
        $validated = $request->validated();
        $quantity = (float) $validated['quantity'];
        $inventory = $item->inventory;
